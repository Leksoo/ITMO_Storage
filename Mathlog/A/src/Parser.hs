...
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parse","Expr","Disj","Conj","Neg","VAR","IMPL","OR","AND","NOT","LEFTBR","RIGHTBR","%eof"]
        bit_start = st * 15
        bit_end = (st + 1) * 15
...
happyReduction_9 (HappyTerminal (Variable happy_var_1))
	 =  HappyAbsSyn7
		 (Var happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	Variable happy_dollar_dollar -> cont 8;
	ImplBin -> cont 9;
	OrBin -> cont 10;
	AndBin -> cont 11;
	NotUn -> cont 12;
